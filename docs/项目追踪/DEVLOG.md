...
### T013 — logger + config_mgr 抽入 yunrong_core 库 ✅

| 日期 | 状态 | 工时 |
|------|:----:|------|
| 2026-06 | ✅ 完成 | 0.5h |

**产出**：`client/src/core/CMakeLists.txt` `client/src/core/infra/logger.*` `client/src/core/infra/config_mgr.*`

| # | 问题 | 原因 | 解决 |
|---|---|---|---|
| — | 无 | — | — |

**操作**：`git mv` 保留历史 → 修 include 路径 → 新建 core/CMakeLists.txt → app 改为链接 yunrong_core。
